...
async function GET(request, { params }) {
    try {
        ...
        let contract = await db_1.db.contract.findUnique({
            where: { id: contractId },
            include: {
        ...
        if (!contract.financialAllocation && (contract.status === "ACCEPTED" || contract.status === "ACTIVE" || contract.status === "COMPLETED")) {
            await (0, contractHelpers_1.backfillFinancialAllocation)(contractId);
            didBackfill = true;
        }
        if (!contract.yield && (contract.status === "ACTIVE" || contract.status === "COMPLETED")) {
            await (0, contractHelpers_1.backfillContractYield)(contractId);
            didBackfill = true;
        }
        // Re-fetch contract if we did backfill to ensure dynamic parameters are computed on actual db values
        if (didBackfill) {
            contract = await db_1.db.contract.findUnique({
                where: { id: contractId },
                include: {
                    financialAllocation: true,
                    yield: true,
            ...
            if (!contract) {
                return server_1.NextResponse.json({ error: "Contract not found after backfill." }, { status: 500 });
            }
        }
        // Calculations
        ...
        const now = new Date();
        const isHarvestOverdue = contract.status === "ACTIVE" &&
        ...
        const hasLowYield = latestProgress?.stage === "HARVEST_COMPLETED" &&
            contract.yield?.fulfillmentStatus === "PARTIAL";
        const monitoring = await (0, contractMonitoring_1.calculateContractMonitoringState)(contractId);
        ...
        const harvestRecorded = contract.yield && contract.yield.actualQuantity !== null
