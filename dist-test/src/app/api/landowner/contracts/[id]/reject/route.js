...
async function PATCH(request, { params }) {
    try {
        ...
        if (contract.landownerId !== user.id) {
            return server_1.NextResponse.json({ error: "Forbidden: You do not own the land associated with this contract." }, { status: 403 });
        }
        if (contract.status !== "PENDING_APPROVAL") {
            return server_1.NextResponse.json({ error: "Only pending contract proposals can be rejected." }, { status: 400 });
        }
