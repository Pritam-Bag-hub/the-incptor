...
async function runTests() {
    ...
    try {
        ...
        const cancelledContract = await prisma.contract.update({
            ...
            data: { status: client_1.ContractStatus.CANCELLED },
        });
        ...
        await prisma.contract.update({
            where: { id: contract1.id },
            data: { status: client_1.ContractStatus.PENDING_APPROVAL },
        });
        ...
        const landownerRequests = await prisma.contract.findMany({
            where: { landownerId: landowner.id, status: client_1.ContractStatus.PENDING_APPROVAL },
        });
        ...
        await prisma.$transaction(async (tx) => {
            ...
            if (!targetLand || targetLand.status !== client_1.LandStatus.AVAILABLE) {
                throw new Error("Land is no longer available.");
            }
            // 2. Lock land status conditionally
            const landLock = await tx.land.updateMany({
                where: { id: land.id, status: client_1.LandStatus.AVAILABLE },
            ...
            await tx.contract.updateMany({
                where: { landId: land.id, status: client_1.ContractStatus.PENDING_APPROVAL, id: { not: contract1.id } },
                data: { status: client_1.ContractStatus.REJECTED, decisionDate: new Date(), notes: "Land accepted under another contract." },
            });
        });
        ...
        if (finalContract1?.status === client_1.ContractStatus.ACCEPTED &&
            finalContract2?.status === client_1.ContractStatus.REJECTED &&
