...
async function getTestCropStages(cropId, cropName) {
...
async function testGenerateMilestonesForContract(contractId, prismaClient) {
...
async function testBackfillContractMilestones(contractId, prismaClient) {
    const contract = await prismaClient.contract.findUnique({
        where: { id: contractId },
    });
    if (!contract)
...
async function runTests() {
    ...
    try {
        ...
        if (!peaCrop) {
            ...
            if (!category)
                throw new Error("No crop category seeded!");
        ...
        const legacyContract = await prisma.contract.create({
            data: {
                ...
                status: client_1.ContractStatus.ACTIVE, // Created directly as ACTIVE without milestones
            },
        });
        ...
        if (countBefore !== 0)
            throw new Error("Milestones initially exist for legacy contract!");
        // Trigger backfill
        ...
        const updatedMs = await prisma.contractMilestone.update({
            ...
            data: {
                status: client_1.MilestoneStatus.COMPLETED,
        ...
        if (updatedMs.status === client_1.MilestoneStatus.COMPLETED && updatedMs.completedAt !== null) {
        ...
        const resetMs = await prisma.contractMilestone.update({
            where: { id: targetMs.id },
            data: {
                status: client_1.MilestoneStatus.IN_PROGRESS,
                completedAt: null,
            },
        });
        if (resetMs.status === client_1.MilestoneStatus.IN_PROGRESS && resetMs.completedAt === null) {
