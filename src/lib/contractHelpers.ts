import { db } from "./db";
import { FulfillmentStatus, MilestoneStatus } from "@prisma/client";
import { getCropStages } from "./cropDataProvider";

export async function backfillFinancialAllocation(contractId: string, tx?: any) {
...
export async function backfillContractYield(contractId: string, tx?: any) {
  const prismaClient = tx || db;
...
export async function generateMilestonesForContract(contractId: string, tx?: any) {
  const prismaClient = tx || db;
  ...
  const stages = await getCropStages(contract.cropId, contract.crop.name);
...
export async function backfillContractMilestones(contractId: string, tx?: any) {
  const prismaClient = tx || db;
