...
export interface NormalizedCropStage {
  title: string;
  sequence: number;
...
/**
 * Interface representing a Crop Data Provider.
 */
interface ICropDataProvider {
  getStages(cropId: string, cropName: string): Promise<NormalizedCropStage[] | null>;
}
 ...
 */
class DatabaseTemplateProvider implements ICropDataProvider {
  async getStages(cropId: string, cropName: string): Promise<NormalizedCropStage[] | null> {
    const templates = await db.cropMilestoneTemplate.findMany({
      where: { cropId },
 ...
 */
class ExternalCropAPIProvider implements ICropDataProvider {
  async getStages(cropId: string, cropName: string): Promise<NormalizedCropStage[] | null> {
    ...
    try {
      ...
      if (data && Array.isArray(data.stages)) {
        return data.stages.map((s: any, idx: number) => ({
          title: s.title,
 ...
 */
class FallbackProvider implements ICropDataProvider {
  async getStages(cropId: string, cropName: string): Promise<NormalizedCropStage[] | null> {
 ...
 */
export async function getCropStages(cropId: string, cropName: string): Promise<NormalizedCropStage[]> {
  ...
  try {
    await db.$transaction(
      finalStages.map((stage) =>
        db.cropMilestoneTemplate.upsert({
          where: {
            cropId_sequence: {
              cropId,
