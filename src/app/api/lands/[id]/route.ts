...
import { SizeUnit } from "@prisma/client";
...
export async function PATCH(
  ...
  try {
    ...
    if (land.status === "UNDER_CONTRACT") {
      return NextResponse.json({ error: "Cannot edit land that is currently UNDER_CONTRACT." }, { status: 400 });
    }
    ...
    if (unit !== undefined) {
      ...
      updateData.unit = unit as SizeUnit;
    }
    ...
    return NextResponse.json({ success: true, land: updatedLand });
  } catch (error: any) {
