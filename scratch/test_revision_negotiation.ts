...
async function run() {
  ...
  try {
    ...
    if (!buyer || !landowner) throw new Error("Missing seeded test users.");
    ...
    if (!rejectRes.ok) throw new Error(`Reject PATCH failed: ${JSON.stringify(rejectData)}`);
    ...
    if (!reviseRes.ok) throw new Error(`Revise POST failed: ${JSON.stringify(reviseData)}`);
    ...
    if (!refreshedContract) throw new Error("Refreshed contract not found in database.");
    ...
    if (refreshedContract.status !== ContractStatus.PENDING_APPROVAL) {
    ...
    if (hist1.revision !== 1 || hist1.status !== ContractStatus.REJECTED || hist1.proposedPrice !== 100000) {
