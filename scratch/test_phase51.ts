...
import { PrismaClient, Role, LandStatus, ContractStatus, FarmProgressStage } from "@prisma/client";
...
async function runTests() {
  ...
  try {
    ...
    const landowner1 = await prisma.user.findFirst({ where: { role: Role.LANDOWNER } });
    ...
    if (contractPending.status !== ContractStatus.ACTIVE) {
    ...
    if (contractAccepted.status !== ContractStatus.ACTIVE) {
    ...
    const progressUpdate2 = await prisma.farmProgress.create({
      data: {
        contractId: contractActive.id,
        stage: FarmProgressStage.SOWING,
    ...
    if (finalHistory.length === 2 && finalHistory[1].stage === FarmProgressStage.SOWING) {
    ...
    const progressUpdate3 = await prisma.farmProgress.create({
      data: {
        contractId: contractActive.id,
        stage: FarmProgressStage.HARVEST_COMPLETED,
    ...
    if (refreshedContract?.status === ContractStatus.ACTIVE) {
