...
async function runTests() {
  ...
  try {
    ...
    if (updatedContract.status === ContractStatus.PENDING_APPROVAL) {
    ...
    if (acceptedLand?.status === LandStatus.UNDER_CONTRACT) {
    ...
    if (activatedContract.status === ContractStatus.ACTIVE) {
    ...
    const completedContract = await prisma.$transaction(async (tx) => {
      ...
      if (!currentContract || currentContract.status !== ContractStatus.ACTIVE) {
    ...
    if (
      completedContract.status === ContractStatus.COMPLETED &&
      completedContract.completedAt !== null &&
      releasedLand?.status === LandStatus.AVAILABLE
    ) {
    ...
    try {
      await prisma.$transaction(async (tx) => {
        ...
        if (!c || c.status !== ContractStatus.ACTIVE) {
    ...
    if (
      (completedContract.status as any) !== ContractStatus.REJECTED &&
      (completedContract.status as any) !== ContractStatus.CANCELLED
    ) {
