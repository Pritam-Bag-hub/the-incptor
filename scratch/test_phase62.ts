...
import { calculateContractMonitoringState, getEffectiveMilestoneStatus } from "../src/lib/contractMonitoring";
...
async function runTests() {
  ...
  try {
    ...
    const buyer = await prisma.user.create({
      data: { phone: "+919999999911", name: "Monitor Buyer", role: Role.BUYER },
    });
    const landowner = await prisma.user.create({
      data: { phone: "+919999999912", name: "Monitor Landowner", role: Role.LANDOWNER },
    });
    const unrelatedUser = await prisma.user.create({
      data: { phone: "+919999999913", name: "Unrelated User", role: Role.BUYER },
    });
    ...
    // Generate milestones
    await generateMilestonesForContract(contract.id, prisma);
    ...
    if (futureMilestone) {
      const effStatus = getEffectiveMilestoneStatus(futureMilestone.plannedDate, futureMilestone.status);
    ...
    if (!seq1) throw new Error("Sequence 1 milestone not found!");
    ...
    const effStatus = getEffectiveMilestoneStatus(updatedSeq1!.plannedDate, updatedSeq1!.status);
    ...
    if (updatedSeq1!.status === MilestoneStatus.PENDING) {
    ...
    const completedEffStatus = getEffectiveMilestoneStatus(completedSeq1!.plannedDate, completedSeq1!.status);
    ...
    await prisma.contractMilestone.update({
      where: { id: seq1.id },
      data: { status: MilestoneStatus.PENDING, completedAt: null },
    });
    // 2. Compute state to generate alert
    await calculateContractMonitoringState(contract.id, prisma);
    ...
    if (!activeAlert) throw new Error("Overdue alert not generated!");
    ...
    const state = await calculateContractMonitoringState(contract.id, prisma);
    ...
    await calculateContractMonitoringState(contract.id, prisma);
    await calculateContractMonitoringState(contract.id, prisma); // double call
    ...
    await generateMilestonesForContract(harvestContract.id, prisma);
    await calculateContractMonitoringState(harvestContract.id, prisma);

    let harvestAlert = await prisma.contractAlert.findFirst({
      where: { contractId: harvestContract.id, type: ContractAlertType.HARVEST_DELAY },
    });
    ...
    const stateActiveOverdue = await calculateContractMonitoringState(contract.id, prisma);
    ...
    await generateMilestonesForContract(onTrackContract.id, prisma);
    const stateActiveOnTrack = await calculateContractMonitoringState(onTrackContract.id, prisma);
    ...
    const stateCompleted = await calculateContractMonitoringState(harvestContract.id, prisma);
