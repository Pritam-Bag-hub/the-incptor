...
import { PrismaClient, Role, LandStatus, ContractStatus, FarmProgressStage, FulfillmentStatus } from "@prisma/client";
...
async function runTests() {
  ...
  try {
    ...
    try {
      ...
      if (contract.status !== ContractStatus.ACCEPTED && contract.status !== ContractStatus.ACTIVE) {
    ...
    const activatedContract = await prisma.$transaction(async (tx) => {
      await tx.contract.update({
        where: { id: contract.id },
        data: { status: ContractStatus.ACTIVE, activatedAt: new Date() },
      });
    ...
    const hasHarvestProgress = progressList.some(p => p.stage === FarmProgressStage.HARVEST_READY || p.stage === FarmProgressStage.HARVEST_COMPLETED);
    ...
    await prisma.farmProgress.create({
      data: {
        contractId: contract.id,
        stage: FarmProgressStage.HARVEST_READY,
    ...
    let statusPartial: FulfillmentStatus = FulfillmentStatus.PENDING;
    if (pctPartial < 90) statusPartial = FulfillmentStatus.PARTIAL;
    ...
    if (yieldPartial.fulfillmentStatus === FulfillmentStatus.PARTIAL && yieldPartial.fulfillmentPercentage === 75.0) {
    ...
    let statusFulfilled: FulfillmentStatus = FulfillmentStatus.PENDING;
    if (pctFulfilled >= 90 && pctFulfilled <= 110) statusFulfilled = FulfillmentStatus.FULFILLED;
    ...
    if (yieldFulfilled.fulfillmentStatus === FulfillmentStatus.FULFILLED && yieldFulfilled.fulfillmentPercentage === 95.0) {
    ...
    let statusOver: FulfillmentStatus = FulfillmentStatus.PENDING;
    if (pctOver > 110) statusOver = FulfillmentStatus.OVERFULFILLED;
    ...
    if (yieldOver.fulfillmentStatus === FulfillmentStatus.OVERFULFILLED && yieldOver.fulfillmentPercentage === 120.0) {
    ...
    if (refContract?.status === ContractStatus.ACTIVE) {
    ...
    await prisma.farmProgress.create({
      data: {
        contractId: contract.id,
        stage: FarmProgressStage.GROWING,
    ...
    if (latest.stage === FarmProgressStage.GROWING) {
    ...
    let pct = 0;
    if (latest.stage === FarmProgressStage.GROWING) pct = 60;
    if (pct === 60) {
    ...
    if (finalContract?.status === ContractStatus.COMPLETED) {
